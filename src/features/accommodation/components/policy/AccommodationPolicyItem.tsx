...
export const AccommodationPolicyItem: FC<AccommodationPolicyItemProps> = (props) => {
  ...
  return (
    <div className="group relative flex items-center gap-3 rounded-md border border-dashed border-slate-500 bg-white px-3 py-2">
      <div className="flex items-center gap-2 text-slate-800">
        <DynamicIcon name={item.icon} strokeWidth={1.6} size={20} />

        <div className="flex flex-1 flex-col gap-1">
          <span className="text-sm font-medium">{item.title}</span>
          <p className="text-justify text-xs font-normal text-slate-600">{item.description}</p>
        </div>
      </div>
